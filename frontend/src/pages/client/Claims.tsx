...
import { getClaimsData } from '@/services/staticDataService';
import { formatCurrency, formatDate } from '@/lib/formatters';
import { getStatusColor, getPolicyIcon } from '@/lib/statusUtils';
import ClaimDetailsModal, { ClaimDetails } from './claims/ClaimDetailsModal';
...
const Claims: React.FC = () => {
  ...
  const loadClaims = useCallback(() => getClaimsData(), []);

  const { data: claims, loading, refetch } = useDataLoader(loadClaims, {
    initialData: [] as ClaimDetails[],
  });
  ...
  const totalPaid = useMemo(
    () => claimsList.filter((c) => c.status === 'PAID').reduce((sum, c) => sum + c.amount, 0),
    [claimsList]
  );

  const handleViewDetails = (claim: ClaimDetails) => {
    setSelectedClaim(claim);
    setShowDetails(true);
  };

  const handleNewClaim = newClaimForm.handleSubmit(async (data: NewClaimData) => {
    try {
