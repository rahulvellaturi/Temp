...
class EmailService {
  ...
  async sendClaimUpdateEmail(email: string, data: ClaimUpdateData): Promise<boolean> {
    const statusColors = {
      APPROVED: '#28a745',
      REJECTED: '#dc3545',
      UNDER_REVIEW: '#ffc107',
    ...
    const statusColor = statusColors[data.status as keyof typeof statusColors] || '#007BFF';
  ...
  async sendPolicyRenewalReminder(email: string, user: User, policyNumber: string, renewalDate: Date): Promise<boolean> {
    const html = `
      <!DOCTYPE html>
      <html>
        <head>
